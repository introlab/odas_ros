...
import threading
# Ros Messages
from odas_msgs.msg import UniqueSSL, Ssl

import time

HOST = 'localhost'      # Symbolic name, meaning all available interfaces
PORTS = [9002]    # Arbitrary non-privileged port
PRINT = True
NUM_SOURCE = 4
TOL = 0.05
...
class Odas:
    ...
    def __init__(self):
        ...
        self.tracked_pub = rospy.Publisher("/SSL", Ssl, queue_size=1)
        # important variables
        self.list_socket = []
        self.nbr_socket = 1
        self.pot_dict = {}
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                for message in test[0:-1:1]:
                    ...
                    self.pot_dict.update({msg.timestamp:msg.src})
                    msg_ = Ssl()
                    pot_full = json.loads(self.pot_dict[msg.timestamp])                    
                    for k in range(len(pot_full)):
                        # When every timestamps are good, we load the information from the msg into a publishable variable
                        source = UniqueSSL()
                        source.x = pot_full[k]["x"]
                        source.y = pot_full[k]["y"]
                        source.z = pot_full[k]["z"]
                        source.E = pot_full[k]["E"]
                    ...
                    self.pot_dict.pop(msg.timestamp)
